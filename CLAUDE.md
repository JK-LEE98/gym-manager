...
> **확인 없이 코드부터 쓰지 않는다. `dev`에 직접 작업하지 않는다.**
> 문서를 포함해 **모든 변경은 브랜치와 PR을 거친다.**
...
문서만 고치는 경우에도 브랜치를 만들고 PR을 올린다. 이슈는 생략할 수 있다.

```
docs/xxx  →  PR  →  dev
```

> **직접 push는 "재시도하면 되는" 상태가 아니다.** CI가 `pull_request`에만 걸려 있어
> push에는 워크플로가 **아예 실행되지 않는다.** 실행되지 않은 체크는 통과도 실패도 아니라
> 영원히 오지 않는 것을 기다리게 된다.
>
> ```
> remote: - 2 of 2 required status checks are expected.
> ```
>
> `on.push`를 추가해도 해결되지 않는다. **push 자체가 먼저 거부**되므로 CI가 돌 기회가 없다.

> **설계 변경이 코드와 같은 PR에 들어가야 할 때는 함께 커밋한다.**
> 예: `feat(pt)` 브랜치에서 `docs/ADR-014`를 함께 수정 →
> PR 하나에 "무엇을 만들었는지"와 "왜 그렇게 만들었는지"가 같이 보인다.
> 문서 전용 PR을 따로 만들지 않아도 되므로 이 편이 낫다.
